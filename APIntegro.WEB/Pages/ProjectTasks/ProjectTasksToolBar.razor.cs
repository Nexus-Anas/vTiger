...
public partial class ProjectTasksToolBar
{
    public async Task ExportDataToExcel()
    {
        if (ProjectTasksList is null) return;
        ...
        var projectsData = ProjectTasksList.Select(p => new[] { p.projecttaskname, p.projecttasktype, p.projecttaskpriority, p.projectid, p.projecttaskprogress, p.startdate, p.enddate, p.projecttaskstatus }).ToList();
