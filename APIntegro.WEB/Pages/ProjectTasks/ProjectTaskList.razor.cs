...
public partial class ProjectTaskList
{
    ...
    private Color GetProgressColor(string value)
    {
        byte progress = Convert.ToByte(value.TrimEnd('%'));

        return progress switch
        {
            byte p when p >= 0 && p <= 30 => Color.Secondary,
            byte p when p > 30 && p <= 60 => Color.Warning,
            byte p when p > 60 && p <= 90 => Color.Tertiary,
            _ => Color.Success
        };
    }
